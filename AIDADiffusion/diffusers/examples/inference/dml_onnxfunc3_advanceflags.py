...
class StableDiffusionPipeline(DiffusionPipeline):
    def __init__(
        ...
        scheduler: Union[DDIMScheduler, PNDMScheduler, LMSDiscreteScheduler, DDPMScheduler],
        safety_checker,
...
def onnxfunc(promtInput, iterationsNum, saveName, userSeedInput, userInputScheduler, userInputGuidance ):
    ...
    pndm = PNDMScheduler(beta_start=0.00085, beta_end=0.012, beta_schedule="scaled_linear", tensor_format='numpy arrays')
    lms = LMSDiscreteScheduler(beta_start=0.00085, beta_end=0.012, beta_schedule="scaled_linear")
    
    if userInputScheduler == "lms":
        pipe = StableDiffusionPipeline.from_pretrained("./stable-diffusion-v1-4", scheduler=lms) #replaced with stablediffusion-v-1-4 git repo #"CompVis/stable-diffusion-v1-4",use_auth_token=True
        torch.manual_seed(seed)
    if userInputScheduler == "ddim":
        pipe = StableDiffusionPipeline.from_pretrained("./stable-diffusion-v1-4", scheduler=ddim) #replaced with stablediffusion-v-1-4 git repo #"CompVis/stable-diffusion-v1-4",use_auth_token=True
        torch.manual_seed(seed)
    if userInputScheduler == "lmsd":
        pipe = StableDiffusionPipeline.from_pretrained("./stable-diffusion-v1-4", scheduler=lmsd) #replaced with stablediffusion-v-1-4 git repo #"CompVis/stable-diffusion-v1-4",use_auth_token=True
        torch.manual_seed(seed)
    if userInputScheduler == "pndm":
        pipe = StableDiffusionPipeline.from_pretrained("./stable-diffusion-v1-4", scheduler=pndm) #replaced with stablediffusion-v-1-4 git repo #"CompVis/stable-diffusion-v1-4",use_auth_token=True
        torch.manual_seed(seed)
    
    userGuidanceScale = userInputGuidance

    prompt = promtInput
    image = pipe(prompt, height=512, width=512, num_inference_steps=iterationsNum, guidance_scale=userGuidanceScale, eta=0.0, execution_provider="DmlExecutionProvider")["sample"][0]
    image.save(saveName)
